...
const HeroModern = ({ movies = [] }) => {
  ...
  return (
    <div className="relative min-h-[70vh] py-12 flex items-center overflow-hidden bg-background text-foreground md:py-8 md:min-h-[auto]">
      {/* Backdrop Background */}
      {currentItem.image && (
      ...
      <div className="relative z-[10] w-full max-w-[1200px] mx-auto px-8 md:px-6 sm:px-4">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center lg:gap-16 md:gap-8">
          ...
          <div className="lg:col-span-8 flex flex-col justify-center space-y-4 order-2 lg:order-1 md:text-center lg:text-left">
            ...
            <div className="flex flex-wrap gap-2 mb-8 md:mb-6 md:justify-center lg:justify-start">
              {currentItem.features.map((feature, index) => (
                <span key={index} className="text-sm text-white bg-card/20 backdrop-blur-sm px-4 py-2 rounded-lg border border-white/30 font-semibold transition-all duration-300 shadow-lg hover:-translate-y-0.5 hover:bg-card/30 hover:border-primary/50 hover:text-primary md:text-xs md:px-3 md:py-1.5">
                  {feature}
                </span>
              ))}
            </div>
            ...
            <div className="flex gap-4 md:flex-row md:gap-3 md:w-full sm:flex-col sm:gap-3">
              ...
              <Link to="/movies?filter=upcoming" className="md:flex-1 sm:w-full">
                <Button
                  size="large"
                  className="h-12 px-8 text-base font-semibold rounded-xl bg-card/20 backdrop-blur-sm border-2 border-white/40 text-white transition-all duration-300 hover:-translate-y-0.5 hover:bg-card/30 hover:border-primary hover:text-white hover:shadow-lg md:w-full md:h-11 md:text-sm sm:h-10 sm:text-sm"
                >
                  Phim Sắp Chiếu
                </Button>
              </Link>
          ...
          <div className="lg:col-span-4 flex justify-center items-center order-1 lg:order-2">
            <div className="relative w-full max-w-[180px] lg:max-w-[220px] rounded-2xl overflow-hidden shadow-2xl transition-all duration-500 hover:scale-105 hover:shadow-3xl">
              ...
              />
              {/* Rating Badge */}
              <div className="absolute top-3 right-3 bg-card/95 backdrop-blur-sm px-2.5 py-1.5 rounded-lg shadow-lg flex items-center gap-1.5">
                <Star className="h-4 w-4 fill-yellow-500 text-yellow-500" />
                <span className="text-foreground font-bold text-base">{currentItem.rating}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-center gap-1.5 mt-8 md:mt-6">
          {displayMovies.map((_, index) => (
            <button
              key={index}
              className={`w-1 h-1 rounded-full border transition-all duration-300 cursor-pointer hover:scale-150 ${currentIndex === index
                ? 'bg-primary border-primary shadow-md shadow-primary/50'
                : 'bg-muted-foreground/30 border-muted-foreground/30 hover:border-primary/50'
                }`}
