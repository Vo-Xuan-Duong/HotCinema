...
const Metric = ({
  ...
  return (
    <div className={cn('min-w-0 space-y-1', className)}>
      {label && <p className="truncate text-[10px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">{label}</p>}
      <div className="flex min-w-0 items-baseline gap-1.5">
        ...
        <p
          className={cn(
            'truncate text-2xl font-semibold tracking-[-0.025em] text-foreground',
        ...
        {trailingContent !== undefined && trailingContent !== null && (
          <span className="shrink-0 text-xs font-medium text-muted-foreground">{sanitizeSlot(trailingContent)}</span>
        )}
      </div>
    </div>
  );
...
const MetricCard = ({
  ...
  <Card className={cn('relative overflow-hidden transition-colors hover:border-primary/35', className)}>
    <span aria-hidden="true" className="absolute inset-y-0 left-0 w-0.5 bg-primary/70" />
    <CardContent className="p-4 pl-[18px]">
      ...
      {trend && trendValue && (
        <p
          className={cn('mt-3 flex items-center gap-1 text-xs font-medium', trend === 'down' && 'text-destructive')}
