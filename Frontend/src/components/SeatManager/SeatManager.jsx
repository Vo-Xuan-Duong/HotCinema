...
const SeatManager = ({ selectedScreen, onSave, onClose }) => {
    ...
    const loadSeatsFromAPI = async (screen) => {
        try {
            ...
            if (seats.length === 0) {
                ...
                console.log('âš ï¸ Phòng chiếu chưa có ghế');
                setSeatLayout({
                    rows: [],
                    totalSeats: 0,
                    vipSeats: [],
    ...
    const generateSeatLayoutFromAPI = (seats) => {
        ...
        layoutSeats.forEach(seat => {
            ...
            if (coordinateMap.has(coordKey)) {
                ...
                console.error(`âš ï¸ TRÙNG TỌA ĐỘ: Ghế ${existing.id} và ${seat.id} cùng có tọa độ (row: ${seat.rowIndex}, col: ${seat.col})`);
            } else {
        ...
        const rows = Object.keys(groupedByRow)
            ...
            .map(rowLabel => {
                ...
                if (colsInRow.length !== uniqueCols.size) {
                    console.error(`âš ï¸ Hàng ${rowLabel} có ghế trùng cột:`, rowSeats.map(s => `${s.id}(col:${s.col})`));
                }
    ...
    const handleBulkEdit = async (values) => {
        try {
            // 📡 GỌI API CẬP NHẬT TỪNG GHẾ ĐàCHỌN (Cần đầy đủ SeatRequest fields)
    ...
    return (
        <div className="font-sans scroll-smooth [&_*::-webkit-scrollbar]:w-2 [&_*::-webkit-scrollbar]:h-2 [&_*::-webkit-scrollbar-track]:bg-gray-100 [&_*::-webkit-scrollbar-track]:rounded [&_*::-webkit-scrollbar-thumb]:bg-gray-400 [&_*::-webkit-scrollbar-thumb]:rounded [&_*::-webkit-scrollbar-thumb]:transition-colors [&_*::-webkit-scrollbar-thumb]:duration-200 hover:[&_*::-webkit-scrollbar-thumb]:bg-background0">
            ...
            {/* Seat Layout */}
            <div className="flex flex-col items-center gap-2 p-5 bg-background rounded-xl border border-border relative mb-5" ref={seatLayoutRef}>
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <Button
                                                            variant="outline"
                                                            size="icon"
                                                            className="add-seat-btn w-8 h-8 border-gray-300 text-muted-foreground opacity-50"
            ...
            <Card className="mb-4">
                ...
                <div className="p-5 flex flex-wrap gap-3">
                    ...
                    <div className="flex items-center gap-1">
                        <div className="w-3.5 h-3.5 bg-background0 rounded flex items-center justify-center text-white">
            ...
            {showBulkModal && (
                <ResponsiveDialog
                    heading="Chỉnh sửa ghế hàng loạt"
                    open={showBulkModal}
            ...
            {showSeatEditModal && selectedSeat && (
                <ResponsiveDialog
                    heading={`Chỉnh sửa ghế ${selectedSeat?.name || `${selectedSeat?.row}${selectedSeat?.number}`}`}
