...
public class BookingStatusHistoryController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<PageResponse<BookingStatusHistoryResponse>>> getAll(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<BookingStatusHistory> pageResult = bookingStatusHistoryService.findAll(pageable);
        Page<BookingStatusHistoryResponse> responsePage = pageResult.map(bookingStatusHistoryMapper::toResponse);
        PageResponse<BookingStatusHistoryResponse> response = PageMapper.toPageResponse(responsePage);
